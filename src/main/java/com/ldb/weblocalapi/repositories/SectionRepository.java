...
@Repository
public interface BorderRepository extends CrudRepository<Section, Section> {
    ...
    @Query(value ="SELECT A.* from SECTION A JOIN USER_LOGIN TUL on A.SEC_ID = TUL.BORDER_IDWHERE USER_NAME = ?", nativeQuery = true)
    List<Section> findByBorderIdFromUserName(String userName);
}
